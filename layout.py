...
def save_config(config_id):
    ...
    if os.path.exists(filename):
        res = input(f"Config {config_id} for {resolution} already exists. Overwrite? [y/n]")
...
def apply_config(config_id):
    ...
    for app in applications:
        ...
        if (
            'x' not in app_state or 'y' not in app_state or
            'w' not in app_state or 'h' not in app_state
        ):
